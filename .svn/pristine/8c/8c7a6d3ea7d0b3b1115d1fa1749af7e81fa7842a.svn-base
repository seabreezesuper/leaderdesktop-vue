      ...
      <section class="form-wrapper" v-show="showLogin">
        ...
        <el-form :model="loginForm" :rules="rules" ref="loginForm">
          ...
          <el-form-item class="button-panel">
            <el-button class="button" @click="onSubmit">{{$t('login.signin')}}</el-button>
          </el-form-item>
        </el-form>
      </section>
    </transition>
...
export default {
  ...
  computed: {
      rules() {
        return{
          userno: [{ required: true, message: this.$t('login.usernomsg'), trigger: "blur" }],
          password: [{ required: true, message: this.$t('login.passwordmsg'), trigger: "blur" }]
        }
      }
  },
  methods: {
    ...
    ajaxLogin: function() {
      ...
      this.axios({
        ...
        .catch(err => {
          ...
          this.$message({ type: "error", message: this.$t('login.catcherrormsg') });//message: "所请求的页面有异常！"
          console.log(err);
        });
    }
  },
